...
void setup() {
  ...
  tft.setRotation(1);
  EEPROM.begin(12);
  delayTime=EEPROM.read(10);

  URTCLIB_WIRE.begin();
  Wire.begin();

  // eeprom.eeprom_read(12, &delayTime);
...
void processSerialRequest(String request) {
  ...
  else if(screen == 4 && request == "esp_goto_restart") {
    uploadScreen();
    delay(200);
    ESP.restart();
    // screen = 4;
...
void startRecording() {
  isRecording = true;
  Serial1.printf("RECORD_/20%02d%02d%02d/%02d%02d%02d.wav\n", rtc.year(), rtc.month(), rtc.day(), rtc.hour(), rtc.minute(), rtc.second());
  Serial.printf("RECORD_/20%02d%02d%02d/%02d%02d%02d.wav\n", rtc.year(), rtc.month(), rtc.day(), rtc.hour(), rtc.minute(), rtc.second());
}

void stopRecording() {
  isRecording = false;
  Serial1.println("STOP_RECORD");
...
void startScreen() {
  ...
  if(Serial1.available() > 0) {
    String report = Serial1.readStringUntil('\n');
    report.trim();
    if (report == "SD_FAIL") {
      tft.fillScreen(BLUE);
      tft.unloadFont();
...
void backFunc() {
  if (screen == 0 && Choose.read() == LOW) {
    ...
    // eeprom.eeprom_write(12, delayTime);
    EEPROM.write(10, delayTime);
    EEPROM.commit();
    tft.fillScreen(BLUE);
    cursor = 0;
    screen = 1;
...
void listFiles(String path) {
  ...
  while (Serial1.available() > 0) {
    String name = Serial1.readStringUntil('\n');
    name.trim();
    if (name.length() > 0 && name != "END_OF_LIST") {
