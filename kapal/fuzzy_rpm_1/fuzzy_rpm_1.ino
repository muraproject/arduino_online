...
// Konstanta ESC untuk motor 1 & 2
const int MIN_PULSE = 90;
const int START_PULSE = 97;
const int MAX_PULSE = 160;
...
// ESP32 komunikasi (tetap dipertahankan untuk kompatibilitas)
HardwareSerial ESP32Serial(PA10, PA9);  // Kita gunakan juga untuk LoRa
...
enum ControlMode { NONE, SMC, FUZZY };
...
// Fungsi konversi nilai write() ke writeMicroseconds() untuk debug
int pulseToMicroseconds(int pulse) {
  // Berdasarkan ESC1.attach(esc1Pin, 1000, 2000)
  // Rentang 0-180 dipetakan ke 1000-2000
  return map(pulse, 0, 180, 1000, 2000);
}
...
void processLoraCommand(const char* command) {
  ...
  if (strlen(command) == 1) {
    ...
    if (cmd == 's') {
      ...
      ESC1.write(currentThrottle1);
      ESC2.write(currentThrottle2);
      ESC3.writeMicroseconds(START_PULSE3_US);
      
      ESP32Serial.println("Motors started at minimum speed");
      ...
      ESP32Serial.println("Motors stopped");
    }
    else if (cmd == '+') {
      // Increase throttle
      ...
      if (throttleIncreased) {
        ESP32Serial.println("Throttle increased");
      } else {
        ESP32Serial.println("Maximum throttle reached");
      }
      ...
      if (throttleDecreased) {
        ESP32Serial.println("Throttle decreased");
      } else {
        ESP32Serial.println("Minimum throttle reached");
      }
    ...
    if (cmd == 'm' || cmd == 'f' || cmd == 't') {
      ...
      if (cmd == 'm') {
        ...
        if (targetRPM > 0) {
          ...
          ESP32Serial.print("SMC mode activated. Target RPM: ");
        ...
        if (targetRPM > 0) {
          ...
          ESP32Serial.print("Fuzzy mode activated. Target RPM: ");
          ESP32Serial.println(targetRPM);
        } else {
          controlActive = false;
          currentMode = NONE;
        ...
        if (targetRPM3 > 0) {
          ...
          // Start motor 3 jika belum berjalan
          ...
          control3Active = false;
          ESP32Serial.println("Third motor control deactivated");
        }
      }
    }
    else {
...
void reportStatusToLora() {
  ...
  ESP32Serial.print(" (");
  ESP32Serial.print(pulseToMicroseconds(currentThrottle1));
  ...
  ESP32Serial.print(" (");
  ESP32Serial.print(pulseToMicroseconds(currentThrottle2));
  ESP32Serial.print("us), RPM: ");
...
void reportThrottle() {
  ...
  Serial.print(" (");
  Serial.print(pulseToMicroseconds(currentThrottle1));
  Serial.print("us), Main2=");
  Serial.print(currentThrottle2);
  Serial.print(" (");
  Serial.print(pulseToMicroseconds(currentThrottle2));
...
void setup() {
  ...
  // Set ESCs to minimum
  ESC1.write(MIN_PULSE);
  ESC2.write(MIN_PULSE);
  ...
  Serial.println("=========================================");
  Serial.println("Triple Control System with I2C and LoRa Ready");
  Serial.println("=========================================");
  ...
  if (controlActive && targetRPM > 0) {
    if (currentTime - lastRPMCheck > RPM_CHECK_INTERVAL) {
      ...
      if (currentMode == SMC) {
        ...
        throttleChange1 = calculateFuzzy(error1, false);
...
// SMC Implementation using integer math
int calculateSMC(int error, int prevError, unsigned long dt_millis, bool isMotor2) {
...
// Sugeno Fuzzy control for motors 1 & 2
int calculateFuzzy(int error, bool isMotor2) {
  ...
  long numerator = (long)mu_NL * z1 + (long)mu_NS * z2 + (long)mu_ZE * z3 + 
                  (long)mu_PS * z4 + (long)mu_PL * z5;
...
float calculateFuzzy3(int error) {
  ...
  // Calculate weighted average (Sugeno defuzzification)
  float numerator = mu_NL * z1 + mu_NS * z2 + mu_ZE * z3 + 
                   mu_PS * z4 + mu_PL * z5;
  int denominator = mu_NL + mu_NS + mu_ZE + mu_PS + mu_PL;
