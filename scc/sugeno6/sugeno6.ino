...
// Pin untuk LCD I2C
#define SDA_PIN 8  // Pin SDA pada ESP32 S3 (ubah sesuai kebutuhan)
#define SCL_PIN 9  // Pin SCL pada ESP32 S3 (ubah sesuai kebutuhan)
...
void updateLCD(PZEM_DATA data, bool isSourceDetected, ChargingMode mode) {
  // Hapus semua isi LCD
  lcd.clear();
  ...
  lcd.print("W PWM:");
  int pwmPercent = (int)((float)pwmValue / pwmMaxValue * 100.0);
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    if (newCurrent >= 0) {
      ...
      // Update LCD saat target arus diubah
      // Ambil data terbaru
      PZEM_DATA data = readPZEM();
      updateLCD(data, sourceDetected, chargingMode);
    }
  }
}
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("ESP32-S3 - Solar Charge Controller (SCC) with PZEM-017 and LCD");
  ...
  // Tampilkan pesan selesai setup pada LCD
  lcd.setCursor(0, 3);
...
bool checkSourceDetection() {
  ...
  if (isSourceDetected != sourceDetected) {
    ...
    // Update LCD saat status sumber berubah
    // Baca data terbaru
    PZEM_DATA data = readPZEM();
    updateLCD(data, sourceDetected, chargingMode);
  }
...
void loop() {
  // Timestamp untuk mengatur pengiriman data WebSocket setiap 1 detik
  ...
  // Update LCD setiap interval yang ditentukan
  if (currentMillis - lastLcdUpdate >= LCD_UPDATE_INTERVAL) {
    lastLcdUpdate = currentMillis;
    // Update LCD dengan data terbaru
    updateLCD(data, isSourceDetected, chargingMode);
  }
  ...
  // Kirim data ke Serial setiap loop
  Serial.print(dataOutput);
  
  // Kirim data ke WebSocket clients hanya setiap 1 detik
  if (currentMillis - lastWebSocketUpdate >= 1000) {  // 1000ms = 1 detik
    ...
    // Clean up inactive WebSocket clients setiap 1 detik juga
    ws.cleanupClients();
  }
  
  // Tunggu sebelum membaca lagi
  delay(100);  // 100ms delay tetap untuk pembacaan dan pemrosesan reguler
}