...
// Variabel untuk mode operasi
bool inConfigMode = false;
unsigned long lastConnectionCheck = 0;
const int CONNECTION_CHECK_INTERVAL = 1000; // Cek koneksi setiap 1 detik
...
void setup() {
  ...
  // Setup WiFi AP tapi jangan matikan setelahnya
  ...
  server.on("/ping", handlePing); // Tambahkan route untuk ping
  server.begin();
...
void loop() {
  // Cek apakah ada perangkat yang terhubung setiap CONNECTION_CHECK_INTERVAL
  if (millis() - lastConnectionCheck > CONNECTION_CHECK_INTERVAL) {
    ...
    if (connectedClients > 0) {
      // Ada klien terhubung, masuk ke mode konfigurasi
      if (!inConfigMode) {
        inConfigMode = true;
        Serial.println("Perangkat terhubung. Masuk ke mode konfigurasi.");
        lcd.clear();
        lcd.setCursor(0, 0);
      ...
      // Tidak ada klien terhubung, masuk ke mode normal
      if (inConfigMode) {
        inConfigMode = false;
        Serial.println("Tidak ada perangkat terhubung. Masuk ke mode normal.");
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print(centerText(welcome_line1));
        lcd.setCursor(0, 1);
  ...
  // Menangani client web - ini harus SELALU berjalan untuk responsivitas
  server.handleClient();
  
  // Jika dalam mode normal, jalankan program lagu ulang tahun
  if (!inConfigMode) {
    runBirthdaySong();
  }
}

void runBirthdaySong() {
  ...
  // Cek koneksi hanya di titik-titik tertentu, bukan setiap delay
  if (checkConnectionQuick()) return;
  
  tone(LEDPin, c, 100); 
  delay(100);
  tone(LEDPin, d, quarter); 
  delay(quarter);
  tone(LEDPin, c, quarter); 
  delay(quarter);
  tone(LEDPin, f, quarter); 
  delay(quarter);
  tone(LEDPin, e, half); 
  delay(half);
  
  // Cek koneksi setelah selesai baris pertama
  if (checkConnectionQuick()) return;
  ...
  tone(LEDPin, c, quarter); 
  delay(quarter);
  tone(LEDPin, g, quarter); 
  delay(quarter);
  tone(LEDPin, f, half); 
  delay(half);
  
  // Cek koneksi setelah selesai baris kedua
  if (checkConnectionQuick()) return;
  ...
  tone(LEDPin, c, 100); 
  delay(100);
  tone(LEDPin, highc, quarter); 
  delay(quarter);
  tone(LEDPin, a, quarter); 
  delay(quarter);
  tone(LEDPin, f, quarter); 
  delay(quarter);
  tone(LEDPin, e, quarter); 
  delay(quarter);
  tone(LEDPin, d, half); 
  delay(half);
  
  // Cek koneksi setelah selesai baris ketiga
  if (checkConnectionQuick()) return;
  ...
  // Cek koneksi setelah selesai loop pertama
  if (checkConnectionQuick()) return;
  ...
  // Cek koneksi setelah selesai baris kelima
  if (checkConnectionQuick()) return;
  ...
  // Cek koneksi setelah selesai baris keenam
  if (checkConnectionQuick()) return;
  ...
  // Cek koneksi setelah selesai baris ketujuh
  if (checkConnectionQuick()) return;
...
// Fungsi untuk cek koneksi secara cepat tanpa menggangu nada
bool checkConnectionQuick() {
  server.handleClient(); // Handle client request
  
  int connectedClients = WiFi.softAPgetStationNum();
  if (connectedClients > 0 && !inConfigMode) {
    inConfigMode = true;
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("  MODE KONFIGURASI");
    ...
    lcd.setCursor(0, 3);
    lcd.print("IP: " + WiFi.softAPIP().toString());
    return true; // Keluar dari fungsi lagu
  }
  return false; // Lanjutkan lagu
}
...
// Handler untuk ping (menjaga koneksi tetap aktif)
void handlePing() {
...
void handleRoot() {
  String html = "<!DOCTYPE html>"
                ...
                "<div class='alert'>Mode konfigurasi aktif. Program lagu akan dilanjutkan setelah semua koneksi WiFi terputus.</div>"
                ...
                "// Keep connection alive - ping server every 5 seconds"
...
void handleSave() {
  if (server.hasArg("welcome_line1") && server.hasArg("welcome_line2") && 
    ...
    if (valid) {
      ...
      // Tampilkan pesan sukses dan kembali ke halaman konfigurasi
      String html = "<!DOCTYPE html>"
                   "<html lang='id'>"
                   "<head>"
                   "<meta charset='UTF-8'>"
                   ...
                   ".message { font-size: 16px; text-align: center; margin-bottom: 30px; }"
                   ...
                   "<p class='message'>Teks telah berhasil disimpan ke perangkat. Perubahan akan terlihat pada saat berikutnya perangkat menampilkan lagu.</p>"
                   "<button onclick='window.location.href=\"/\"'>Kembali ke Konfigurasi</button>"
                   "</div>"
                   "</body>"
                   "</html>";
