...
void loop() {
  ...
  if (currentMillis - lastDebounceTime > debounceDelay) {
    // Detect tempo mode - STOP + NEXT or STOP + PREV
    if (!currentStop) {
      if (!currentNext || !currentPrev) {
        // Enter tempo mode if not already in it
        if (!isTempoMode) {
          isTempoMode = true;
          updateOLEDDisplay();
        }
        ...
        if (currentMillis - lastTempoAdjustTime > tempoAdjustDelay) {
          if (!currentNext) { // Increase tempo (decrease microseconds)
            ...
            updateOLEDDisplay();
          } else if (!currentPrev) { // Decrease tempo (increase microseconds)
      ...
      // Exit tempo mode if we were in it and stop button is released
      if (isTempoMode) {
        isTempoMode = false;
        updateOLEDDisplay();
      }
      
      // Normal button handling when not in tempo mode
