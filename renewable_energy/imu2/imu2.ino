...
void setup() {
  ...
  Serial.println(F("=== SOLAR TRACKER KALIBRASI ==="));
...
void hitungPosisiMatahari() {
  ...
  float targetRoll = rollTimur + (proporsiHari * (rollBarat - rollTimur));
  
  // Vertikal: Selatan (musim dingin) ke Utara (musim panas)
  float targetPitch = pitchSelatan + (proporsiBulan * (pitchUtara - pitchSelatan));
  ...
  // Simpan target dalam bentuk roll/pitch untuk tracking
  targetRoll = targetRoll;
  targetPitch = targetPitch;
  
  Serial.print(F("Proporsi - Hari:"));
  Serial.print(proporsiHari * 100, 0);
  Serial.print(F("% Bulan:"));
  Serial.print(proporsiBulan * 100, 0);
  Serial.println(F("%"));
  Serial.print(F("Target Roll:"));
  Serial.print(targetRoll, 1);
  Serial.print(F(" Pitch:"));
  Serial.println(targetPitch, 1);
}

void jalankanTracking() {
  // STEP 1: TRACKING HORIZONTAL (TIMUR-BARAT) berdasarkan jam
  if (!horizontalSelesai) {
    ...
    if (abs(errorRoll) > toleransi) {
      if (errorRoll > 0) {
        // Target roll lebih besar, gerak ke arah roll positif
        ...
        Serial.print(F(">> TIMUR Roll:"));
      } else {
        // Target roll lebih kecil, gerak ke arah roll negatif  
        digitalWrite(motorHPin1, HIGH);
        digitalWrite(motorHPin2, LOW);
        digitalWrite(motorVPin1, LOW);  // Pastikan vertikal off
        digitalWrite(motorVPin2, LOW);
        Serial.print(F(">> BARAT Roll:"));
      }
      Serial.print(roll, 1);
      ...
      horizontalSelesai = true;
      waktuMulaiStep = millis();
      Serial.print(F(">>> HORIZONTAL SELESAI (Roll:"));
      Serial.print(roll, 1);
      Serial.println(F("°) <<<"));
      Serial.println(F("Step 2: Tracking VERTIKAL (Proporsi Bulan)..."));
    }
  ...
  // STEP 2: TRACKING VERTIKAL (UTARA-SELATAN) berdasarkan bulan
  if (!vertikalSelesai) {
    ...
    if (abs(errorPitch) > toleransi) {
      if (errorPitch > 0) {
        // Target pitch lebih besar, gerak ke arah pitch positif
        digitalWrite(motorHPin1, LOW);  // Pastikan horizontal off
        digitalWrite(motorHPin2, LOW);
        digitalWrite(motorVPin1, HIGH);
        digitalWrite(motorVPin2, LOW);
        Serial.print(F(">> UTARA Pitch:"));
      } else {
        // Target pitch lebih kecil, gerak ke arah pitch negatif
        digitalWrite(motorHPin1, LOW);  // Pastikan horizontal off
        digitalWrite(motorHPin2, LOW);
        digitalWrite(motorVPin1, LOW);
        digitalWrite(motorVPin2, HIGH);
        Serial.print(F(">> SELATAN Pitch:"));
      }
      Serial.print(pitch, 1);
      Serial.print(F("° Target:"));
      Serial.print(targetPitch, 1);
      Serial.print(F("° Err:"));
      Serial.println(errorPitch, 1);
    } else {
      stopMotor();
      vertikalSelesai = true;
      sedangTracking = false;
      Serial.print(F(">>> VERTIKAL SELESAI (Pitch:"));
      Serial.print(pitch, 1);
      Serial.println(F("°) <<<"));
